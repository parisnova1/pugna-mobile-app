    ...
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <CenteredColumn>
          ...
          <Button
            label={loading ? t('login.pleaseWait') : t('login.logIn')}
            uppercase={false}
            onPress={submit}
            disabled={loading}
            style={styles.submit}
...
const styles = StyleSheet.create({
  ...
  close: { position: 'absolute', top: -20, right: 0, padding: 6, zIndex: 1 },
  eyebrow: { fontFamily: FONT_DISPLAY_BOLD, fontSize: 11, letterSpacing: 3, color: ACCENT, textTransform: 'uppercase', marginBottom: 8 },
  title: { fontFamily: FONT_DISPLAY_BOLD, fontSize: 32, color: TEXT, marginBottom: 28 },
